...
func (s *Service) List(ctx context.Context, params ListWalletsParams) ([]Wallet, error) {
...
func (s *Service) CreditPaymentCapture(ctx context.Context, params CreditPaymentCaptureParams) error {
	...
	if params.Amount <= 0 || strings.TrimSpace(params.Country) == "" || strings.TrimSpace(params.Currency) == "" {
		return ErrInvalidWalletRequest
	}
